...
def refresh_apps(just_delete = False):
    ...
    for D in src_folders:
        for i in glob.glob(os.path.join(D, "applications", "*.desktop")):
            ...
            texec = os.path.join(target_folder, desktop_prefix + fpk + ".sh")
            ...
            if icon is not None:
                ...
                if icon:
                    ...
                    d.set_icon_path(n)
            # create executable for every desktop file to work around
            # lipstick launcher handling of the same "instance"
            # windows. as it tracks Exec in .desktop, we will change
            # it for every app to a unique one
            with open(texec, "w") as f:
                f.write("#!/bin/sh\nexec flatpak-runner %s\n" % fpk)
            os.chmod(texec, stat.S_IRWXU | stat.S_IRGRP | stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
            d.save_processed(tfile, texec)
